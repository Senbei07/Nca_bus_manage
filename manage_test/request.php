...
function renderTables() {
    ...
    Object.entries(request).forEach(([br_id, obj]) => {
        ...
        reqArr.forEach((qr_request, idx) => {
            ...
            const timePlusVal = timePlusArr[idx] || '90';
            html += `<tr>
                <td>${idx + 1}</td>
                <td>${code}</td>
                ...
                <td><input type="number" class="form-control" name="time_plus[${br_id}][]" value="${timePlusVal}" onchange="onTimePlusChange('${br_id}', ${idx}, this)" min="0"></td>
                <td>
        ...
        html += `<tr>
            <td>ใหม่</td>
            <td>${br_id}-3-ใหม่</td>
            <td>${createSelect('', '0', routeOptions, br_id, 'request', reqArr.length, false)}</td>
        ...
        html += `<tr>
            <td>ใหม่</td>
            <td>${br_id}-1-ใหม่</td>
            <td>${createSelect('', '0', routeOptions, br_id, 'reserve', reserveArr.length, false)}</td>
...
function insertRow(br_id, type, idx, pos) {
    ...
    if (type === 'request') {
        ...
        if (!request[br_id].time_plus) request[br_id].time_plus = [];
        request[br_id].time_plus.splice(insertIdx, 0, '90');
    }
