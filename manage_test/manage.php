                                                        ...
                                                        <?php if (!empty($break_list)): ?>
                                                            <ul class="list-group main-break-list" id="main-break-list-<?php echo htmlspecialchars($go_route); ?>" data-go-route="<?php echo htmlspecialchars($go_route); ?>" data-br-id="<?php echo htmlspecialchars($br_id); ?>">
                                                                <?php foreach ($break_list as $index => $item): ?>
                                                                    <li class="list-group-item d-flex justify-content-between align-items-center break-driver-item"
                                                                        draggable="true"
                                                                        data-break-index="<?php echo $index; ?>"
                                                                        data-go-route="<?php echo htmlspecialchars($go_route); ?>"
                                                                        data-br-id="<?php echo htmlspecialchars($br_id); ?>">
                                                        ...
                                                        <?php if (!empty($ex_not_ready_list)): ?>
                                                            <ul class="list-group ex-break-list" id="ex-break-list-<?php echo htmlspecialchars($br_id); ?>" data-br-id="<?php echo htmlspecialchars($br_id); ?>">
                                                                <?php foreach ($ex_not_ready_list as $index => $item): ?>
                                                                    <li class="list-group-item d-flex justify-content-between align-items-center ex-driver-item"
                                                                        draggable="true"
                                                                        data-break-index="<?php echo $index; ?>"
                                                                        data-br-id="<?php echo htmlspecialchars($br_id); ?>">
                                                                        <?php echo htmlspecialchars($item['em_name']); ?>
                                                        ...
                                                        <?php if (!empty($coach_not_ready_list)): ?>
                                                            <ul class="list-group coach-break-list" id="coach-break-list-<?php echo htmlspecialchars($br_id); ?>" data-br-id="<?php echo htmlspecialchars($br_id); ?>">
                                                                <?php foreach ($coach_not_ready_list as $index => $item): ?>
                                                                    <li class="list-group-item d-flex justify-content-between align-items-center coach-driver-item"
                                                                        draggable="true"
                                                                        data-break-index="<?php echo $index; ?>"
                                                                        data-br-id="<?php echo htmlspecialchars($br_id); ?>">
                                                                        <?php echo htmlspecialchars($item['em_name']); ?>
        ...
        <script>
                    ...
                    tbody.querySelectorAll('tr').forEach((row, rowIndex) => {
                        ...
                        row.addEventListener('drop', function(e) {
                            ...
                            if (data.type === 'main') {
                                ...
                                // นำ mainDriver ไปใส่ jsMainBreak[goRoute] ถ้า goRoute เดิม = brId ให้ unshift, ถ้าไม่ใช่ให้ push ไปสายใหม่
                                if (!jsMainBreak[goRoute]) jsMainBreak[goRoute] = [];
                                jsMainBreak[goRoute].unshift(mainDriver);
                                // Recalculate queues
                                const lastIndex = jsPlan[brId].length - 1;
                                jsPlan[brId].forEach((row, idx) => {
                                    row.new_queue = (idx === lastIndex) ? `${brId}-3-last` : `${brId}-3-${idx + 1}`;
                                });
                                jsMainBreak[goRoute].forEach((driver, idx) => {
            ...
            if (planForm) {
                planForm.addEventListener('submit', function(e) {
                    // Use the updated jsPlan object for submission
