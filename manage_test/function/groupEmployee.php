...
?>
 <script>
    console.log('time', <?php echo json_encode($time); ?>);
 </script>
<?php


    return [$goto, $re, $main, $main_re, $break, $return_request, $time, $pr_ids];
}
 ...
 */
function getEmployee($conn, $route, $goto, $num, $x, $type, $return_request, $time) {
    $route_in = '(' . implode(',', $route) . ')';
    $Gather = [3, 4];
    ...
    ?>
    <script>
        console.log('newdata', <?php echo json_encode($data_new ?? []); ?>);
        console.log('data', <?php echo json_encode($data); ?>);
        console.log('merge_old', <?php echo json_encode($merge_old ?? []); ?>);
    </script>
    <?php
